...
```
Benchmark               Mode  Cnt  Score   Error  Units
MainBenchmark.isolated    ss   10  1.205 ± 0.262   s/op
MainBenchmark.boot        ss   10  1.210 ± 0.229   s/op
MainBenchmark.manual      ss   10  1.235 ± 0.180   s/op
MainBenchmark.func        ss   10  0.641 ± 0.135   s/op
```
...
* "func" registers all beans in the application directly, by class or
...
Note that `ConfigurationClassPostProcessor` still accounts for 6% of
...
We suspect that the difference between "isolated" (vanilla) and "boot"
...
(it should have improved the "isolated" sample as much or more).
...
```
Benchmark               Mode  Cnt  Score   Error  Units
MainBenchmark.isolated    ss   10  1.173 ± 0.209   s/op
MainBenchmark.boot        ss   10  1.184 ± 0.186   s/op
MainBenchmark.manual      ss   10  1.016 ± 0.183   s/op
```
...
```
Benchmark           Mode  Cnt  Score   Error  Units
MainBenchmark.func    ss   10  0.590 ± 0.129   s/op
```
