...
import com.example.config.StandardMetadataReaderFactory;
...
public class AnnotationProcessingInterceptor {
    ...
    @Around("within(org.springframework.context.annotation.ConfigurationClassPostProcessor) && "
            + "execution( * setMetadataReaderFactory(..))")
    public void metadata(ProceedingJoinPoint joinPoint) throws Throwable {
        if (logger.isDebugEnabled()) {
            logger.debug(joinPoint.toShortString());
        }
        joinPoint.proceed(new Object[] { new StandardMetadataReaderFactory() });
    }
